...
def test_task_list_view(client):
    ...
    assert 'todo/task_list.html' in [t.name for t in response.templates]  # تغییر 'template_name' به 'templates'
...
def test_task_create_view(client):
    ...
    response = client.post(reverse('todo:task-create'), {
        'title': 'New Task',
...
def test_task_update_view(client):
    ...
    response = client.post(reverse('todo:task-update', kwargs={'pk': task.pk}), {
        'title': 'Updated Task',
